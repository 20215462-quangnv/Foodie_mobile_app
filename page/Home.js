...
const HomeScreen = ({ navigation }) => {
  ...
  const loadUserProfile = async () => {
    try {
      console.log("=== Home Screen Profile Debug Logs ===");
      const response = await getUserProfile();
      console.log("1. Full Response:", response);
      console.log("2. Profile Data:", response.data);
      console.log("3. First Letter:", response.data?.fullName?.charAt(0));
      setUserProfile(response.data);
    } catch (error) {
      console.error("4. Error loading profile:", error);
    }
  };
